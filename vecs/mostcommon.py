...
while 1:
    lens = []
    word1 = input("Word: ").lower()#.split(' ')#king,-,man
    vec1 = word2Vec[word1]
    #vec1 = word2Vec[word1[0]]+word2Vec[word1[2]] if word1[1] == '+' else word2Vec[word1[0]]-word2Vec[word1[2]]
    #vec1 += word2Vec[input("Vec 2: ")]
    for word, vec in word2Vec.items():
        if len(lens) < 10:
            lens.append((round(findCos(vec1,vec),8),word))#[mag((vec1-vec).rep), word])
        else:
            #distance = mag((vec1-vec).rep)
            angle = round(findCos(vec1,vec),8)
            if angle > lens[-1][0]:
                lens.pop()
                lens.append([angle,word])
        #print(str(lens),end='\r')
        lens = sortLict(lens)[::-1]
    print(lens)
    #input('')
    ...
    '''
    #print(findCos(vector1.rep, vector2.rep))
